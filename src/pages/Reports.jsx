...
const initialReports = [
  { id: 1, item: 'Black Wallet', status: 'Lost', location: 'Library', date: '2026-09-05' },
  { id: 2, item: 'Water Bottle (Blue)', status: 'Found', location: 'Canteen', date: '2026-09-06' },
  { id: 3, item: 'Notebook', status: 'Lost', location: 'Room 204', date: '2026-09-06' },
  { id: 4, item: 'Wired Earphones', status: 'Found', location: 'Sports Ground', date: '2026-09-07' },
  { id: 5, item: 'Spectacles Case', status: 'Lost', location: 'Cafeteria', date: '2026-09-07' },
]
...
function Reports() {
  ...
  // Escape key se modal band karna
  useEffect(() => {
    const handleEsc = (e) => {
  ...
  // Filter (status) + search dono ek saath apply
  const filteredReports = reports
    ...
    .sort((a, b) => new Date(b.date) - new Date(a.date)) // newest date sabse upar

  const handleChange = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value })
    setErrors({ ...errors, [e.target.name]: '' }) // type karte hi error hata do
  }
  ...
  const handleSubmit = (e) => {
    ...
    const newReport = {
      id: Date.now(), // temporary unique id — backend aane par ID wahan se aayegi
      ...formData,
    }

    setReports([newReport, ...reports])
    setFormData({ item: '', status: 'Lost', location: '', date: '' })
    setErrors({})
    setShowForm(false)
    setShowSuccess(true)
    setTimeout(() => setShowSuccess(false), 2000) // 2 sec baad success msg hide
  }
    ...
    <div className="min-h-screen bg-white font-['Neue_Montreal',_sans-serif]">
      ...
      <div className="text-center px-6 md:px-16 py-10">
        <h1 className="text-2xl md:text-4xl font-bold text-gray-900">
          All <span className="text-[#FF6D29]">Reports</span>
        </h1>
        <p className="text-gray-600 mt-2">Browse all lost and found item reports on campus.</p>
      </div>
      ...
      {showSuccess && (
        <div className="flex justify-center mb-4">
          <div className="flex items-center gap-2 bg-green-50 text-green-700 text-sm px-4 py-2 rounded-lg">
            <CheckCircle size={16} /> Report added successfully!
          </div>
        </div>
      )}
      ...
      {showForm && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-lg font-bold text-gray-900 mb-4">Add New Report</h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3" noValidate>
              <div>
                <input
                  ...
                  className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none ${
                    errors.item ? 'border-red-400' : 'border-gray-300 focus:border-[#FF6D29]'
                  }`}
                />
              ...
              <div>
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none ${
                    errors.location ? 'border-red-400' : 'border-gray-300 focus:border-[#FF6D29]'
                  }`}
                />
              ...
              <div>
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none ${
                    errors.date ? 'border-red-400' : 'border-gray-300 focus:border-[#FF6D29]'
                  }`}
                />
              ...
              <button
                type="submit"
                className="mt-2 bg-[#FF6D29] text-white py-2 rounded-lg text-sm font-medium hover:bg-[#e85f20] transition-colors"
              >
                Submit Report
              </button>
            </form>
      ...
      <div className="px-6 md:px-16 pb-20 max-w-3xl mx-auto flex flex-col gap-4">
        {filteredReports.map((report) => (
          ...
          >
            <div className="flex items-center gap-3">
              <Search className="text-[#FF6D29]" size={20} />
              <div>
        ...
        {filteredReports.length === 0 && (
          <p className="text-center text-gray-400 mt-10">No reports found.</p>
        )}
      </div>
